...
class Chat {
    ...
    static async getRoom(roomId) {
        const data = await ref_1.Ref.chatRoomsDoc(roomId).get();
        return data.data();
    }
    /**
     ...
     */
    static async sendWelcomeMessage(uid) {
        const system = await setting_model_1.Setting.getSystemSettings();
        if (system.helperUid && system.welcomeMessage) {
            const chatRoomId = [system.helperUid, uid].sort().join("-");
            // 웰컴 메시지를 채팅방 메세지로 기록
            const message = {
                chatRoomDocumentReference: ref_1.Ref.chatRoomsCol.doc(chatRoomId),
                senderUserDocumentReference: ref_1.Ref.userDoc(system.helperUid),
                text: system.welcomeMessage,
                timestamp: admin.firestore.Timestamp.now(),
            };
            await ref_1.Ref.chatRoomMessagesCol.add(message);
            ...
            const info = {
                ...
                users: [message.senderUserDocumentReference, ref_1.Ref.userDoc(uid)],
            };
    ...
    static async getOtherUserUidsFromChatMessageDocument(data) {
        ...
        // remove my uid
        const uidsExceptMe = uids.filter((uid) => uid !== data.senderUserDocumentReference.id);
        // get uids of disabled users
        const uidsOfDisabledUsers = chatRoomData.notificationDisabledUsers.map((ref) => ref.id);
        // remove disabled users
        const userUids = uidsExceptMe.filter((uid) => !uidsOfDisabledUsers.includes(uid));
